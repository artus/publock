...
function copying_MessageChain_results_in_exact_copy() {
    let mc1 = new MessageChain_1.MessageChain();
    let key = new NodeRSA({ b: 512 });
    let mc2 = MessageChain_1.MessageChain.copyMessageChain(mc1);
    ...
    // Add a new message 
    let message = new Message_1.Message("John", "This is my first message.", new Date().toISOString(), "", mc1.lastMessage.hash, key.exportKey('public'));
    message.encryptedHash = message.generateEncryptedHash(key.exportKey('private'));
    message.hash = message.generateHash();
    let messageHash = message.hash;
    mc1.addMessage(message);
    mc2 = MessageChain_1.MessageChain.copyMessageChain(mc1);
